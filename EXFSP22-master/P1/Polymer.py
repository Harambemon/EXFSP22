...
class macroMolecule():
    def __init__(self, n, m, l):
        ...
        """
        self.m = m #segmentLength
        self.n = n # degreeOfPolymerization
        self.l = l # merWt
        self.l = 14
        self.n =1000 #degree of polymerization or number of links
        self.MW = self.n*self.l
        self.m=0.154E-9
    ...
    def freelyJointedChainModel(self):
        ...
        M=int(self.n)
        for i in range(M):
            m=molecule(molecularWeight=self.l)
            m.MW += 1 if (i==0 or i==(self.n-1)) else 0
            m.position = lastPosition.getRndPosOnSphere(self.m)
            self.mers.append(m)
    ...
    if __name__ == "__main__": # calling the main code
        n = int(input("degree of polymerization(1000)?: "))
        m = int(input("How many molecules(50)?: "))
        # list = [n,m]
        obj = dc(n)
        om = dc(m)
        list = [obj, om]
        list.freelyJointedChainModel()
        # the output
        print("Metrics for 50 molecules of degree of polymerization =", obj.N)
        values = dc.array(obj.centerOfMass.getTup())
        values *= 10 ** 9
        print("Average Center of Mass(nm) =", values)
        print("End-to-end distance(um): ")
        print(" Average =", obj.endToEndDistance * 10 ** 6)
        values = []
        for i in obj.mers:
            values.append(i.position.mag())
        print(" Std. Dev =",dc.std(values) * 10 ** 6)
        print("Radius of gyration(um): ")
        print(" Average =", obj.radiusOfGyration * 10 ** 6)
        print(" Std. Dev =", obj.radiusOfGyration * 10 ** 6 / obj.MW ** 0.5)